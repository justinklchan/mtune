...
fname='MTUNEPractice-ConventionalVSSmartp_DATA_2024-03-07_0144'
...
def parse_summary(fname,f,con,snr_thresh,band_thresh,noise_thresh):
	sm_out=0
	con_out=0
	lines=f.split('\n')
	elts0=lines[0].split(',')
	...
	# print ('"'+fname[:-12]+'",')
	return sm_out,con_out,result,noise_result,noise_result2
			...
			# for line in lines[6:7]:
				...
				if len(con_right)>0 and result_code[con_right]!='Incomplete' and \
					...
					if out:
						...
						sm_out_elt,con_out_elt,sm_right,noise_result,noise_result2=parse_summary(out,fsummary,con_right,snr_thresh,band_thresh,noise_thresh)
				...
				if len(con_left)>0 and result_code[con_left]!='Incomplete' and \
					...
					if out:
						...
						out2=out2.replace('csv','txt')
						checkfit=np.loadtxt ('../kenya_files/'+out2)[-1]
						sm_out_elt,con_out_elt,sm_left,noise_result,noise_result2=parse_summary(out,fsummary,con_left,snr_thresh,band_thresh,noise_thresh)
						sm_out.append(sm_out_elt)
						con_out.append(con_out_elt)
						if noise_result2=='retry':
							# print (out,'retry')
